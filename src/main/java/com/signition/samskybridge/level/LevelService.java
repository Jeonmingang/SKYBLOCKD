...
public class LevelService {
    ...
    public void onPlace(org.bukkit.Material m, org.bukkit.Location l, org.bukkit.entity.Player p){ /* block-xp removed */ } }
                if (!ok) return;
            }
        } catch (Throwable ignored) {}
    ...
    public long requiredXpForLevel(int level){
        try{
            org.bukkit.configuration.file.FileConfiguration cfg = com.signition.samskybridge.Main.get().getConfig();
    ...
    public void tryLevelUp(com.signition.samskybridge.data.IslandData is){
        while(is.getXp() >= requiredXpForLevel(is.getLevel())){
            is.setLevel(is.getLevel()+1);
            org.bukkit.entity.Player p = org.bukkit.Bukkit.getPlayer(is.getId());
            if (p != null){
                String msg = com.signition.samskybridge.Main.get().getConfig().getString("messages.level.leveled-up","&a레벨업! 현재 레벨: &f<level>");
                p.sendMessage(com.signition.samskybridge.util.Text.color(msg.replace("<level>", String.valueOf(is.getLevel()))));
            }
        }
    }
    public void applyXpPurchase(com.signition.samskybridge.data.IslandData is, long add){
        is.addXp(add);
        tryLevelUp(is);
    }
