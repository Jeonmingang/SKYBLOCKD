...
public class UpgradeService {
    ...
    private ItemStack buildSizeItem(Player p){
        ...
        im.setDisplayName(Text.color(plugin.getConfig().getString("xp.name", "&e경험치 구매")));
        java.util.List<String> cfg = plugin.getConfig().getStringList("xp.lore");
        java.util.List<String> lore = new java.util.ArrayList<>();
        int amount = plugin.getConfig().getInt("upgrade.xp.amount", plugin.getConfig().getInt("xp.amount", 50));
        double cost = plugin.getConfig().getDouble("upgrade.xp.cost", plugin.getConfig().getDouble("xp.cost", 1000.0));
        ...
        if (cfg != null && !cfg.isEmpty()){
            ...
            lore.add(Text.color("&7구매량: &f" + amount + " XP"));
