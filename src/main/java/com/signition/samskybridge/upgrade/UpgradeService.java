...
private void appendNextLevelLore(org.bukkit.inventory.meta.ItemMeta meta, String typeKey, int nextLevel) {
    ...
    lore.add(color(this.plugin.getConfig().getString("upgrades-ui.templates." + typeKey + ".next-header", "&e다음 레벨 효과:")));
    java.util.List<String> add = lvl.getStringList("lore");
    for (String line : add) lore.add(color(line));
    meta.setLore(lore);
}
...
private boolean isIslandOwner(org.bukkit.entity.Player p) {
    try {
        return com.signition.samskybridge.feature.featureService.islandOwner(p);
    } catch (Throwable t) {
