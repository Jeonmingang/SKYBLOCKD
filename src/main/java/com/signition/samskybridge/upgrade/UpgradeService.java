...
public class UpgradeService {
    ...
    public void openGui(Player p){
        ...
        inv.setItem(slotSize, named(new ItemStack(Material.WHITE_STAINED_GLASS), plugin.getConfig().getString("gui.size-item-name","섬 크기 업그레이드"),
                new String[]{
                    "현재 반지름: " + is.getSize() + " (단계 " + (currentSizeStep(is.getSize())+1) + "/" + (maxSizeStep()==1000?"∞":String.valueOf(maxSizeStep()+1)) + ")",
                    "다음 반지름: " + nextSizeValue(is.getSize()) + " | 가격: " + (long)costSize(is.getSize()) + " | 요구 레벨: " + needLevelSize(is.getSize()),
                    "좌클릭=돈 구매, 쉬프트=레벨 구매"
                }));
        inv.setItem(slotTeam, named(new ItemStack(Material.PLAYER_HEAD), plugin.getConfig().getString("gui.team-item-name","팀원 수 업그레이드"),
                new String[]{
                    "현재 팀 최대: " + is.getTeamMax() + " (단계 " + (currentTeamStep(is.getTeamMax())+1) + "/" + (maxTeamStep()==1000?"∞":String.valueOf(maxTeamStep()+1)) + ")",
                    "다음 팀 최대: " + nextTeamValue(is.getTeamMax()) + " | 가격: " + (long)costTeam(is.getTeamMax()) + " | 요구 레벨: " + needLevelTeam(is.getTeamMax()),
                    "좌클릭=돈 구매, 쉬프트=레벨 구매"
                }));
        p.openInventory(inv);
    }
