...
public class ManagementGui {
  ...
  public void open(Player p){
    ...
    int idx = 0;
    inv.setItem(idx++, head(is.getOwner(), "&c섬장", java.util.Arrays.asList(Text.color("&7좌클릭: 관리"))));
    for (UUID u : is.getCoOwners()) inv.setItem(idx++, head(u, "&6부섬장", java.util.Arrays.asList(Text.color("&7좌클릭: 관리"))));
    for (UUID u : is.getMembers()) inv.setItem(idx++, head(u, "&f섬원", java.util.Arrays.asList(Text.color("&7좌클릭: 관리"))));
    p.openInventory(inv);
  }
  private ItemStack head(UUID u, String name, java.util.List<String> lore){
