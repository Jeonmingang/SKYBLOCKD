...
public class ManagementGui {
  ...
  private ItemStack make(Material m, String name, String... lore){
    ItemStack it = new ItemStack(m); ItemMeta im = it.getItemMeta(); im.setDisplayName(Text.color(name));
    java.util.List<String> ls = new java.util.ArrayList<>(); for (String s : lore) ls.add(Text.color(s)); im.setLore(ls); it.setItemMeta(im); return it;
  }
  public void onClick(Player p, InventoryClickEvent e){
    if (e.getInventory().getTitle()==null || !e.getInventory().getTitle().contains(TITLE)) return;
    e.setCancelled(true);
