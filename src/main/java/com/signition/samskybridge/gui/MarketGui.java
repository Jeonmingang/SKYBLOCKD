...
import org.bukkit.Bukkit; import org.bukkit.Material; import org.bukkit.OfflinePlayer; import org.bukkit.entity.Player; import org.bukkit.inventory.Inventory; import org.bukkit.inventory.ItemStack; import org.bukkit.inventory.meta.ItemMeta; import org.bukkit.inventory.meta.SkullMeta;
public class MarketGui { private final Main plugin; private final DataStore store; public MarketGui(Main plugin, DataStore store){ this.plugin=plugin; this.store=store; }
  public void open(Player p){ Inventory inv=Bukkit.createInventory(p,54, Text.color("&b섬 매물")); for (IslandData is: store.getAll()){ if (!is.isForSale()) continue; inv.addItem(head(is)); } p.openInventory(inv); }
  private ItemStack head(IslandData is){
    ItemStack it=new ItemStack(Material.PLAYER_HEAD); ItemMeta im=it.getItemMeta();
    if (im instanceof SkullMeta){
      SkullMeta sm=(SkullMeta)im; OfflinePlayer op=Bukkit.getOfflinePlayer(is.getOwner()); sm.setOwningPlayer(op);
      ...
      int sizeDelta = Main.get().getConfig().getInt("upgrade.size.delta",5);
      int teamDelta = Main.get().getConfig().getInt("upgrade.team.delta",1);
