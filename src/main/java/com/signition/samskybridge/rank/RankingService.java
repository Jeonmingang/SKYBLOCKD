...
private void applyPrefix(Player p, int rank){
    ...
    public java.util.List<com.signition.samskybridge.data.IslandData> getSortedIslands(){
        java.util.List<com.signition.samskybridge.data.IslandData> out = new java.util.ArrayList<>(this.store.all());
        java.util.Collections.sort(out, java.util.Comparator.comparingInt(com.signition.samskybridge.data.IslandData::getLevel)
            .thenComparingLong(com.signition.samskybridge.data.IslandData::getXp).reversed());
        return out;
    }
}