...
public IslandData findByOwnerName(String name){
    for (IslandData v : islands.values()){
        if (v.getName() != null && v.getName().equalsIgnoreCase(name)) return v;
    }
    return null;
}
...
public Collection<IslandData> all(){ return islands.values(); }
    ...
    public java.util.List<java.util.UUID> listPlayers(){
        java.util.Set<java.util.UUID> set = new java.util.HashSet<java.util.UUID>();
        try {
            java.util.Map<?,?> m = this.map; // preferred
            if (m != null){
                for (Object k : m.keySet()){
                    if (k instanceof java.util.UUID) set.add((java.util.UUID) k);
                    else try { set.add(java.util.UUID.fromString(String.valueOf(k))); } catch (Throwable ignored){}
                }
            }
        } catch (Throwable ignored){}
        try {
            java.util.Map<java.util.UUID, com.signition.samskybridge.data.IslandData> m2 =
                (java.util.Map<java.util.UUID, com.signition.samskybridge.data.IslandData>) this.getClass().getDeclaredField("data").get(this);
            if (m2 != null) set.addAll(m2.keySet());
        } catch (Throwable ignored){}
        return new java.util.ArrayList<java.util.UUID>(set);
    }
    public long getXP(java.util.UUID id){
        try {
            com.signition.samskybridge.data.IslandData d = get(id);
            if (d != null) return d.getXp();
        } catch (Throwable ignored){}
        return 0L;
    }
    public void addXP(java.util.UUID id, long delta){
        ...
        try {
            com.signition.samskybridge.data.IslandData d = get(id);
            if (d == null){
                d = new com.signition.samskybridge.data.IslandData(id);
            }
            d.setXp(next);
            put(d);
            save();
