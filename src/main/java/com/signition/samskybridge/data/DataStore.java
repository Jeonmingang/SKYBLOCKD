...
public synchronized IslandData get(java.util.UUID owner) {
    return islands.get(owner);
}

/**
 * Get island by owner or create a new one with sensible defaults.
 * Kept for backward compatibility with 20-branch callers.
 */
public synchronized IslandData getOrCreate(java.util.UUID owner, java.lang.String ownerName) {
    IslandData d = islands.get(owner);
    if (d == null) {
        d = new IslandData(owner, ownerName, 1, 0L, 0, 0);
        islands.put(owner, d);
        save();
    }
    return d;
