...
private final Object ioLock = new Object();
  ...
  private org.bukkit.scheduler.BukkitTask pendingMarketSaveTask;
...
public void saveAsync(){
  synchronized(ioLock){
    if (pendingSaveTask == null || pendingSaveTask.isCancelled()){
      pendingSaveTask = org.bukkit.Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, new Runnable(){
        public void run(){
          synchronized(ioLock){
...
public void save(){
  ...
  public void setChatOn(java.util.UUID u, boolean on){ if (on) chatOn.add(u); else chatOn.remove(u); }
  public void toggleChat(java.util.UUID u){ if (!chatOn.add(u)) chatOn.remove(u); }
}

  public java.util.Set<java.util.UUID> getAllOwners(){ return new java.util.HashSet<java.util.UUID>(islands.keySet()); }


public int purgeOldSales(int maxAgeDays){
  long now = System.currentTimeMillis();
  long cutoff = now - (long)maxAgeDays*24L*60L*60L*1000L;
  int before = market.size();
  market.removeIf(s -> s.listedAt < cutoff);
  if (market.size() != before) saveMarketAsync();
  return before - market.size();
}


public void saveNow(){
  synchronized(ioLock){
    if (pendingSaveTask != null){ try{ pendingSaveTask.cancel(); }catch(Exception ignore){} pendingSaveTask = null; }
    save();
  }
}


public void saveMarketNow(){
  synchronized(ioLock){
    if (pendingMarketSaveTask != null){ try{ pendingMarketSaveTask.cancel(); }catch(Exception ignore){} pendingMarketSaveTask = null; }
    saveMarket();
  }
}
