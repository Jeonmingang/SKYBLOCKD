...
this.levelService = new LevelService(this, dataStore);
        ...
        try { org.bukkit.plugin.RegisteredServiceProvider<net.milkbowl.vault.economy.Economy> rsp = getServer().getServicesManager().getRegistration(net.milkbowl.vault.economy.Economy.class); if (rsp != null) { com.signition.samskybridge.gui.UpgradeGUI ug = new com.signition.samskybridge.gui.UpgradeGUI(this, levelService, rsp.getProvider()); getServer().getPluginManager().registerEvents(ug, this); this.getServer().getPluginManager().registerEvents(ug, this); this.getLogger().info("UpgradeGUI enabled."); } else { getLogger().warning("Vault Economy not found. XP 구매 GUI 비활성화."); } } catch (Throwable t) { getLogger().warning("UpgradeGUI init failed: "+t.getMessage()); }
    }
...
private boolean openUpgradeOrForward(org.bukkit.entity.Player p){
    ...
    if (getConfig().getBoolean("xp-purchase.enabled", true) && rsp != null){
        try {
            com.signition.samskybridge.gui.UpgradeGUI ug = new com.signition.samskybridge.gui.UpgradeGUI(this, levelService, rsp.getProvider());
            ug.open(p);
            guiOpened = true;
