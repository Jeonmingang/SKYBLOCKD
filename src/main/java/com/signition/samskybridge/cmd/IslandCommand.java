...
void sendHelp(Player p, int page){
        ...
        if (page == 1){
            ...
            p.sendMessage(Text.color("&7/섬 채팅 &f: 섬 전용 채팅 토글"));
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0]){
            case "채팅": { boolean on = plugin.getChat().toggle(p); p.sendMessage(on ? "§a섬 채팅 ON" : "§c섬 채팅 OFF"); return true; }
            case "페이지": {
                int pg = 1;
            ...
            case "설치": { p.sendMessage(Text.color("&c이 기능은 제거되었습니다.")); return true; }
            case "제거": { p.sendMessage(Text.color("&c이 기능은 제거되었습니다.")); return true; }
...
case "레벨": {
    ...
    int _team = is.getTeamMax();p.sendMessage(Text.color("&a[업그레이드] &7크기: &f"+_size+" &7인원: &f"+_team));return true;
}
