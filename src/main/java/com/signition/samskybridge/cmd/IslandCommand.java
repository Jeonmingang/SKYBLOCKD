...
public class IslandCommand implements CommandExecutor {
    public IslandCommand(Main plugin,
                         ...
                         com.signition.samskybridge.level.LevelService levelService,
                         com.signition.samskybridge.upgrade.UpgradeService upgradeService,
                         com.signition.samskybridge.rank.RankingService rankingService,
                         com.signition.samskybridge.feature.FeatureService featureService) {
        this.plugin = plugin;
        this.store = store;
        this.levelService = (levelService != null) ? levelService : plugin.getLevelService();
        this.level = this.levelService;
        this.upgradeService = (upgradeService != null) ? upgradeService : plugin.getUpgradeService();
        this.upgrade = this.upgradeService;
        this.rankingService = (rankingService != null) ? rankingService : plugin.getRankingService();
        this.ranking = this.rankingService;
        this.featureService = (featureService != null) ? featureService : plugin.getFeatureService();
    }
    public IslandCommand(Main plugin,
                         com.signition.samskybridge.data.DataStore store,
                         com.signition.samskybridge.level.LevelService levelService,
                         com.signition.samskybridge.upgrade.UpgradeService upgradeService,
                         com.signition.samskybridge.rank.RankingService rankingService) {
        this(plugin, store, levelService, upgradeService, rankingService, plugin.getFeatureService());
    }


    private final com.signition.samskybridge.level.LevelService levelService;
    private final com.signition.samskybridge.upgrade.UpgradeService upgradeService;
    private final com.signition.samskybridge.rank.RankingService rankingService;
    private final com.signition.samskybridge.feature.FeatureService featureService;
...
void sendHelp(Player p, int page){
        ...
        if (page == 1){
            ...
            p.sendMessage(Text.color("&7/섬 리로드 &f: 설정 리로드 (config.yml)"));
            p.sendMessage(Text.color("&7/섬 채팅 &f: 섬 전용 채팅 토글 (/is chat)"));
            ...
            p.sendMessage(Text.color("&7/섬 탈퇴 &f: 현재 섬에서 탈퇴 (=/is team leave)"));
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) { sendHelp(p, 1); return true; }
            if ("리로드".equals(args[0]) || "reload".equalsIgnoreCase(args[0])){
                plugin.reloadConfig();
                try { if (plugin.getChatService()!=null) plugin.getChatService().reload(); } catch(Throwable ignore){}
                p.sendMessage(Text.color("&a구성 리로드 완료."));
                return true;
            }
        switch (args[0]){
            case "페이지": {
...
case "탈퇴": {
    // delegate to BentoBox/BSkyBlock
    p.performCommand("is team leave");
    return true;
}
case "설치": {
