...
private void sendLevelWithTemplate(org.bukkit.entity.Player p, com.signition.samskybridge.data.IslandData is) {
    ...
    int level = this.levelService.getLevel(is);
    long xp = this.levelService.getXP(is);
    long next = this.levelService.getNextNeeded(is);
    int progress = (int)Math.min(100, Math.round((xp * 100.0) / Math.max(1, next)));
    int sizeLevel = this.upgradeService.getSizeLevel(is);
    int memberLevel = this.upgradeService.getMemberLevel(is);
    int mineLevel = com.signition.samskybridge.feature.FeatureService.getMineLevel(is.getId());
    int farmLevel = com.signition.samskybridge.feature.FeatureService.getFarmLevel(is.getId());
...
private void sendRankingWithTemplate(org.bukkit.entity.Player p, int page) {
    ...
    for (int i = from; i < to; i++) {
        ...
        int lvl = this.levelService.getLevel(is);
        long xp = this.levelService.getXP(is);
        long next = this.levelService.getNextNeeded(is);
        int upgProgress = 0;
        if (showUpg) {
            ...
            int haveSum = this.upgradeService.getSizeLevel(is) + this.upgradeService.getMemberLevel(is)
                        + com.signition.samskybridge.feature.FeatureService.getMineLevel(is.getId()) + com.signition.samskybridge.feature.FeatureService.getFarmLevel(is.getId());
