...
public class FeatureService {
    // --- owner mapping for mines (location -> island owner UUID)
    private final java.util.Map<String, java.util.UUID> mineOwnerIndex = new java.util.concurrent.ConcurrentHashMap<>();
    private String keyOf(org.bukkit.Location loc){ return loc.getWorld().getName()+":"+loc.getBlockX()+","+loc.getBlockY()+","+loc.getBlockZ(); }
    ...
    public boolean isInMineRegion(Player p, Location loc){
        ...
        UUID ownerId = resolveOwnerByLocation(loc);
        if (ownerId == null) return false;
        if (!isOwnerOrMember(p, loc)) return false;
        Location origin = getLoc(ownerId, "mine");
        if (origin == null || origin.getWorld() == null || loc.getWorld() == null) return false;
        if (!origin.getWorld().equals(loc.getWorld())) return false;
        int x = loc.getBlockX(), z = loc.getBlockZ();
        return x >= origin.getBlockX() - 6 && x <= origin.getBlockX() + 6
            && z >= origin.getBlockZ() - 6 && z <= origin.getBlockZ() + 6;
    }
...
// Register/unregister mine base location to owner UUID (call on install/uninstall)
public void registerMineAt(org.bukkit.Location base, java.util.UUID ownerUuid){
    if (base!=null && ownerUuid!=null) mineOwnerIndex.put(keyOf(base), ownerUuid);
}
public void unregisterMineAt(org.bukkit.Location base){
    if (base!=null) mineOwnerIndex.remove(keyOf(base));
}


// Resolve island owner UUID by a mine location; internal index first, then Bento/BSkyBlock reflection fallback.
public java.util.UUID resolveOwnerByLocation(org.bukkit.Location loc){
    try{
        java.util.UUID found = mineOwnerIndex.get(keyOf(loc));
    ...
    try{
        ...
        Object islands = bbClazz.getMethod("getIslands").invoke(bb);
        try{
            java.lang.reflect.Method getIslandAt = islands.getClass().getMethod("getIslandAt", org.bukkit.Location.class);
            Object island = getIslandAt.invoke(islands, loc);
            if (island != null){
                java.lang.reflect.Method getOwner = island.getClass().getMethod("getOwner");
                Object owner = getOwner.invoke(island);
                if (owner != null) return java.util.UUID.fromString(String.valueOf(owner));
            }
        }catch(NoSuchMethodError|NoSuchMethodException ex){/* ignore */}
    }catch(Throwable ignored){}
    // BSkyBlock fallback
    try{
        Class<?> api = Class.forName("world.bentobox.bskyblock.api.BSkyBlockAPI");
        Object inst = api.getMethod("getInstance").invoke(null);
        java.lang.reflect.Method getIslandAt = api.getMethod("getIslandAt", org.bukkit.Location.class);
        Object island = getIslandAt.invoke(inst, loc);
        if (island != null){
            java.lang.reflect.Method getOwner = island.getClass().getMethod("getOwner");
            Object owner = getOwner.invoke(island);
            if (owner != null) return java.util.UUID.fromString(String.valueOf(owner));
...
// Check if player is owner or member of island that owns mine at location.
public boolean isOwnerOrMember(org.bukkit.entity.Player p, org.bukkit.Location loc){
    java.util.UUID owner = resolveOwnerByLocation(loc);
    if (owner == null) return false;
    try{
        if (islandOwner(p)) return true;
    }catch(Throwable ignored){}
    try{
        if (islandMember(p)) return true;
    }catch(Throwable ignored){}
    // Fallback: if player's island owner id equals 'owner'
    try{
        java.util.UUID pid = p.getUniqueId();
        java.util.UUID po = findIslandOwnerId(pid);
        if (owner.equals(po)) return true;
    }catch(Throwable ignored){}
    return false;
}

// Attempt to find island owner id (by player) using internal DataStore via reflection to avoid API dependency
public java.util.UUID findIslandOwnerId(java.util.UUID anyMember){
    try{
        Object store = this.store != null ? this.store : null;
        if (store != null){
            try{
                java.lang.reflect.Method m = store.getClass().getMethod("getIslandByPlayer", java.util.UUID.class);
                Object is = m.invoke(store, anyMember);
                if (is != null){
                    java.lang.reflect.Method m2 = is.getClass().getMethod("getOwner");
                    Object owner = m2.invoke(is);
                    if (owner != null) return java.util.UUID.fromString(String.valueOf(owner));
                }
            }catch(NoSuchMethodException nsme){
                try{
                    java.lang.reflect.Method m = store.getClass().getMethod("findByOwner", java.util.UUID.class);
                    Object is = m.invoke(store, anyMember);
                    if (is != null){
                        java.lang.reflect.Method m2 = is.getClass().getMethod("getOwner");
                        Object owner = m2.invoke(is);
...
// Use owner UUID as regen/cooldown key; fallback to breaker UUID if unresolved.
public java.util.UUID mineKeyFor(org.bukkit.Location loc, org.bukkit.entity.Player breaker){
    java.util.UUID owner = resolveOwnerByLocation(loc);
    return owner != null ? owner : breaker.getUniqueId();
}