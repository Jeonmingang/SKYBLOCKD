...
private boolean consumeOne(Player p, Material mat){
    ...
    public static int getMineLevel(java.util.UUID id) {
        return INSTANCE != null ? INSTANCE.getMineLevel(id) : 0;
    }
    public static int getFarmLevel(java.util.UUID id) {
        return INSTANCE != null ? INSTANCE.getFarmLevel(id) : 0;
    }

    public boolean isIslandWorld(org.bukkit.World world) {
        if (world == null) return false;
        try {
            Class<?> bbClazz = Class.forName("world.bentobox.bentobox.BentoBox");
            Object bb = bbClazz.getMethod("getInstance").invoke(null);
            Object gm = bbClazz.getMethod("getGameModeManager").invoke(bb);
            java.util.Collection<?> gms = (java.util.Collection<?>) gm.getClass().getMethod("getGameModes").invoke(gm);
            String wName = world.getName();
            for (Object g : gms) {
                Object worlds = g.getClass().getMethod("getWorlds").invoke(g);
                if (worlds instanceof java.util.Collection) {
                    for (Object w : (java.util.Collection<?>) worlds) {
                        if (wName.equalsIgnoreCase(String.valueOf(w))) return true;
                    }
                }
            }
        } catch (Throwable ignore) {}
        String name = world.getName().toLowerCase(java.util.Locale.ROOT);
        return name.contains("bskyblock") || name.contains("island");
    }

    public boolean islandOwner(org.bukkit.entity.Player p) {
        if (p == null) return false;
        try {
            Class<?> bbClazz = Class.forName("world.bentobox.bentobox.BentoBox");
            Object bb = bbClazz.getMethod("getInstance").invoke(null);
            Object islands = bbClazz.getMethod("getIslands").invoke(bb);
            Object opt = islands.getClass().getMethod("getIslandByOwner", java.util.UUID.class).invoke(islands, p.getUniqueId());
            if (opt != null) return true;
        } catch (Throwable ignore) {}
        try {
            com.signition.samskybridge.data.IslandData is = this.getIslandByPlayer(p.getUniqueId());
            return is != null && java.util.Objects.equals(is.getOwner(), p.getUniqueId());
        } catch (Throwable ignore) {}
        return false;
    }

    public boolean islandMember(org.bukkit.entity.Player p) {
        if (p == null) return false;
        try {
            Class<?> bbClazz = Class.forName("world.bentobox.bentobox.BentoBox");
            Object bb = bbClazz.getMethod("getInstance").invoke(null);
            Object islands = bbClazz.getMethod("getIslands").invoke(bb);
            Object opt = islands.getClass().getMethod("getIslandByPlayer", java.util.UUID.class).invoke(islands, p.getUniqueId());
            if (opt != null) return true;
        } catch (Throwable ignore) {}
        try {
            com.signition.samskybridge.data.IslandData is = this.getIslandByPlayer(p.getUniqueId());
    ...
    public int getMaxMineLevel() {
        org.bukkit.plugin.Plugin pl = org.bukkit.Bukkit.getPluginManager().getPlugin("SamSkyBridge");
        if (pl != null) {
            org.bukkit.configuration.file.FileConfiguration cfg = ((org.bukkit.plugin.java.JavaPlugin)pl).getConfig();
            return cfg.getInt("feature.mine.max-level", 5);
        }
        return 5;
    }
    ...
    public int getMaxFarmLevel() {
        ...
        if (pl != null) {
            org.bukkit.configuration.file.FileConfiguration cfg = ((org.bukkit.plugin.java.JavaPlugin)pl).getConfig();
            return cfg.getInt("feature.farm.max-level", 5);
        }
        return 5;
    }
